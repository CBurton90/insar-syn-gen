...
def test_synthetic_completion(config):
    
    config = munch.munchify(toml.load(config))
    input_path = config.combine.root_dir
    output_path = config.interpolation.output_path
    tol = config.interpolation.tolerance
    
    for set in range(1,3):

        file_list = glob(input_path + 'unwrapped/set'+str(set)+'/*.npy')
        ...
        for file in file_list:
            ...
            fname = file.split('/')[-1].split('.npy')[0]
            out = output_path + 'unwrapped/set'+str(set)+'/'+'Delaunay_'+fname+'.png'
            plt.imsave(out, final, cmap='jet', vmin=-20, vmax=20)

            if config.interpolation.wrapped:
                insar_wrapped = np.mod(final, 2*np.pi) - np.pi
                insar_wrapped = (insar_wrapped - np.min(insar_wrapped)) / np.ptp(insar_wrapped) # min-max normalize
                fname = file.split('/')[-1].split('.npy')[0].split('unwrapped')[-1]
                out = output_path + 'wrapped/set'+str(set)+'/'+'Delaunay_wrapped'+fname+'.png'
                plt.imsave(out, insar_wrapped, cmap='jet', vmin=0, vmax=1)

if __name__ == '__main__':
    config = '/home/conradb/git/insar-syn-gen/configs/insar_synthetic_vel.toml'
    test_synthetic_completion(config)
    
    #np.random.randn(20, 5) V = np.random.randn(15, 5)R =
    #np.random.randn(20, 15) + np.dot(U, V.T)
